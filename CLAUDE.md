...
4. `cli.py` reads `databricks.yml` and resolves `domain` and `catalog` for the target env via `resolve_bundle_var` (target-level override → bundle-level default).
...
7. Consumer runs `databricks bundle deploy --target dev`.
...
`--env` is passed to `lakeflow-generate` and substituted for `${env}` in YAML string values (e.g., S3 paths). `domain` and `catalog` are resolved from `databricks.yml` at generation time via `resolve_bundle_var` — set them once under `variables:` (bundle default) or per-target under `targets.<env>.variables`.

### Governance (Dual-Layer)

- **TBLPROPERTIES** — embedded in SQL at table creation time (versioned with code): `Domain`, `GitHubRepo`, `FrameworkUsed`, `Layer`, `PipelineName`, `JobName`.
...
- `columns` entries may optionally include `tags` (a dict of key-value pairs applied via `ALTER TABLE … ALTER COLUMN … SET TAGS` in the `2_apply_uc_tags` task).
